...
class MultiRobotRegistration:
    ...
    def __init__(self,vin:int,
        ...
        for i in range(3):
            frame = Keyframe(True,
                        None, 
                        pose223(gtsam.Pose2(0,0,0)),
                        index=i)
            frame.ring_key = np.sum(np.ones((self.bearing_bins,self.range_bins)) * float('inf'), axis = 0)
    ...
    def global_pose_optimization_routine(self,source_points,source_pose,target_points,target_pose,point_noise=0.5):
    ...
    def compare_frames(self,frame_self:Keyframe,frame_multi_robot:Keyframe):
        ...
        multi_robot_points = frame_multi_robot.submap # get the points of the outside robot
        ...
        if self.use_count:
            if len(multi_robot_points) <= self.min_points or len(my_robot_points) <= self.min_points:
                return None, False, "Not enough points " + str(len(multi_robot_points)) + " " + str(len(my_robot_points))

        # catch the ratio of the point clouds
        if self.use_ratio:
            if((len(multi_robot_points) / len(my_robot_points)) < (1/self.points_ratio) or 
                    (len(my_robot_points) / len(multi_robot_points)) > self.points_ratio):  
                return None, False, "Ratio Wrong " + str((len(multi_robot_points) / len(my_robot_points))) + " " + str(len(my_robot_points) / len(multi_robot_points)) 

        # gate the registration based on scan context results
        if self.use_context:
            if (np.sum(abs(frame_self.context - frame_multi_robot.context)) > self.max_scan_context):
                return None, False, "Context High " + str(np.sum(abs(frame_self.context - frame_multi_robot.context)))

        # build the subroutine and run the global optimizer to perform global ICP
        subroutine, pose_samples = self.global_pose_optimization_routine(multi_robot_points, 
                                                                                    gtsam.Pose2(0,0,0),
                                                                                    my_robot_points,
                                                                                    gtsam.Pose2(0,0,0))
        ...
        r_mtx = Rotation.from_euler('xyz', [0,0,-global_result.x[2]]).as_matrix()[:2, :2] 
        reg_points = multi_robot_points.dot(r_mtx) + np.array([global_result.x[0], global_result.x[1]])
        ...
        fit_score = np.mean(distances[distances < float("inf")]) # we need fit score to derive a covariance
        ...
        reg_points = reg_points.dot(icp_x_form[1][:2, :2].T) + np.array([icp_x_form[1][0][2], icp_x_form[1][1][2]])
        ...
        r_mtx_icp = np.row_stack((np.column_stack((icp_x_form[1][:2, :2].T, [0,0])), [0,0,1]))
        theta_icp = Rotation.from_matrix(r_mtx_icp).as_euler("xyz")[2]

        # build the pose chain between frames
        global_opti = gtsam.Pose2(global_result.x[0],global_result.x[1],-global_result.x[2])
        ...
        pose_global = pose_global + np.array([pose_between_frames.x(), pose_between_frames.y()])
        pose_global = pose_global.dot(pose_slam.matrix()[:2, :2])
        pose_global = pose_global + np.array([pose_slam.x(), pose_slam.y()])
        pose_global = gtsam.Pose2(pose_global[0][0], 
                                    pose_global[0][1], 
                                    -(pose_between_frames.theta() + pose_slam.theta()))

        # log the outcomes
        return [overlap, fit_score, pose_between_frames, pose_global, None], True, "Good"
