...
class MultiRobotSLAM(SLAMNode):
    ...
    def init_multi_robot(self, ns) -> None:
        ...
        self.point_compression_resolution = rospy.get_param(
            ns + "point_compression_resolution"
        )

        # multi-robot PCM
        self.multi_robot_pcm_queue_size = rospy.get_param(
            ns + "multi_robot_pcm_queue_size"
        )
        ...
        self.mrr_max_translation_search = rospy.get_param(
            ns + "plcs/max_translation_search"
        )
        ...
        self.ring_key_sub = rospy.Subscriber(
            RING_KEY_TOPIC, RingKey, callback=self.ring_key_callback, queue_size=50
        )

        # point cloud request exchange
        self.request_pub = rospy.Publisher(
            DATA_REQUEST_TOPIC, DataRequest, queue_size=5
        )
        ...
        self.key_frame_pub = rospy.Publisher(
            KEYFRAME_TOPIC, KeyframeImage, queue_size=5
        )
        self.key_frame_sub = rospy.Subscriber(
            KEYFRAME_TOPIC,
            KeyframeImage,
            callback=self.keyframe_callback,
            queue_size=50,
        )

        # loop closure exchange
        self.loop_closure_pub = rospy.Publisher(
            LOOP_CLOSURE_TOPIC, LoopClosure, queue_size=5
        )
        self.loop_closure_sub = rospy.Subscriber(
            LOOP_CLOSURE_TOPIC,
            LoopClosure,
            callback=self.loop_closure_callback,
            queue_size=20,
        )
        ...
        self.dummy_pub = rospy.Publisher(
            "/" + self.rov_id + "/slam/global_reg_run", Dummy, queue_size=5
        )
        self.dummy_sub = rospy.Subscriber(
            "/" + self.rov_id + "/slam/global_reg_run",
            Dummy,
            callback=self.global_registration_callback,
            queue_size=50,
        )

        # shutdown sub
        self.shutdown_sub = rospy.Subscriber(
            "/" + self.rov_id + "/shutdown",
            Dummy,
        ...
        self.merged_pub = rospy.Publisher("merged", PointCloud2, queue_size=5)
    ...
    def init_message_pool(self) -> None:
        ...
        for robot, k in zip(robots, keys):
            ...
            self.message_pool[robot] = MultiRobotRegistration(
                vin=self.vin,
    ...
    def ring_key_callback(self, msg: RingKey) -> None:
        ...
        self.lock.acquire()
        self.message_pool[msg.vin].add_keyframe_ring_key(
            frame
        )  # log the keyframe into our message pool
        ...
        distances, indexes = tree.query(
            frame.ring_key,
            k=self.mrr_k_neighbors,
            distance_upper_bound=self.mrr_max_tree_cost,
        )
        ...
        if home[msg.vin] is not None and len(distances) != 0:
            frame_pose = home[msg.vin].compose(
                frame.source_pose
            )  # get this pose in our reference frame
            indexes, distances = self.filter_by_distance(
                frame_pose, indexes, distances, keyframes_
            )
        ...
        if len(distances) != 0:  # are there any valid neighbors
            ...
            vals_to_request = []
            for i in range(
                msg.keyframe_id - 1, msg.keyframe_id + 2
            ):  # loop over the ids in the submap
                if (
                    i not in self.outside_frames[msg.vin]
                ):  # check if we have this point cloud aleady
                    vals_to_request.append(
                        i
                    )  # add it to the list of point clouds we need
    ...
    def request_callback(self, msg: DataRequest) -> None:
        ...
        for i in list(msg.data):
            ...
            msg = KeyframeImage()
            msg.cloud = n2r(
                np.c_[frame.points, np.zeros_like(frame.points[:, 0])], "PointCloudXYZ"
            )  # push the point cloud
    ...
    def keyframe_callback(self, msg: KeyframeImage) -> None:
        ...
        points = ros_numpy.point_cloud2.pointcloud2_to_xyz_array(msg.cloud)[
            :, :2
        ]  # decode the cloud
        ...
        pose_their_frame = gtsam.Pose2(
            pose_their_frame[0], pose_their_frame[1], pose_their_frame[3]
        )  # make it a gtsam object
        keyframe_id = msg.keyframe_id
        vin = msg.vin

        # perform the updates
        self.lock.acquire()
        self.message_pool[vin].update_keyframe(
            pose_their_frame, points, keyframe_id
        )  # update the regisgtration system with this cloud
        self.outside_frames[vin][
            keyframe_id
        ] = keyframe_id  # update the hash table to indicate we have recived this message, do not request it again
        self.lock.release()
    ...
    def global_registration_callback(self, msg: Dummy) -> None:
        ...
        job_status, loop = self.run_global_registration(
            my_keys, their_keys, vin_list, my_keyframes, message_pool_copy
        )
        ...
        if job_status == False:
            if (
                len(self.frames_to_check) != 0
            ):  # if there are more jobs, send a message to run this callback again
                self.dummy_pub.publish(Dummy())
            self.lock.release()
            return
        ...
        if self.use_pcm:
            while (
                self.multi_robot_queue[loop.vin]
                and loop.target_key - self.multi_robot_queue[loop.vin][0].target_key
                > self.multi_robot_pcm_queue_size
            ):
                self.multi_robot_queue[loop.vin].pop(0)
        queue = list(
            self.multi_robot_queue[loop.vin]
        )  # copy the queue before we release the lock
        ...
        if len(pcm) > 0:
            self.lock.acquire()
            self.publish_multi_robot_registration(
                self.vis_pcm_result(pcm, loop.vin)
            )  # vis pcm results
    ...
    def loop_closure_callback(self, msg: LoopClosure) -> None:
        ...
        if (msg.vin, msg.target_key) not in self.outside_frames_added:
            ...
            icp_transform = n2g(
                np.array(list(msg.data)), "Pose2"
            ).inverse()  # make sure to get the inverse
            ...
            target_pose = self.keyframes[msg.source_key].pose.compose(
                icp_transform
            )  # get the pose in this robots frame
            self.keyframes_multi_robot.append(
                Keyframe(
                    True,
                    None,
                    pose223(target_pose),
                    ros_numpy.point_cloud2.pointcloud2_to_xyz_array(msg.cloud)[:, :2],
                    source_pose=self.message_pool[msg.vin]
                    .keyframes[msg.target_key]
                    .source_pose,
    ...
    def state_update_callback(self, msg: PoseHistory) -> None:
        ...
        data = np.reshape(
            data, (len(data) // 3, 3)
        )  # reshape the message into a Nx3 trajectory [x,y,theta]
        iter = np.min(
            [len(self.message_pool[msg.vin].keyframes), len(data)]
        )  # loop over whatever is shorter, the state update or keyframes
        for i in range(iter):  # pass the new state vector of to the registration system
            self.message_pool[msg.vin].keyframes[i].source_pose = gtsam.Pose2(
    ...
    def query_tree(self) -> None:
        ...
        for vin in self.robots:  # loop over the other robots in the system
            keys_i = self.message_pool[
                vin
            ].get_ring_keys()  # get the ring keys from this robot
            keys += keys_i  # add these keys to the list
            for i in range(len(keys_i)):
                vin_list.append(
                    vin
                )  # create an array of vin numbers to go along with each key
        ...
        if len(keys) > 0:  # if we have any ring keys at all
            ...
            for id in indexes:
                ...
                job_indexes.append(
                    real_indexes[id]
                )  # get the index in the vin.keyframes

            # euclidan space filtering
            if len(job_indexes) != 0:
                vin_for_jobs, job_indexes, distances = self.filter_by_distance_2(
                    self.keyframes[-2].pose,
            ...
            if len(job_indexes) != 0:
                self.frames_to_check.append(
                    ([len(self.keyframes) - 2], job_indexes, vin_for_jobs)
                )
    ...
    def get_scan_context(self, points: np.array) -> np.array:
        ...
        r_dis = np.array((r_cont / self.sc_max_range) * self.sc_range_bins).astype(
            np.uint16
        )  # discret coords
        b_dis = np.array(
            (((b_cont / self.sc_max_bearing) + 1) / 2) * self.sc_bearing_bins
        ).astype(np.uint16)
    ...
    def update_submaps(self) -> None:
        ...
        if (
            len(self.keyframes) >= 3
        ):  # only do so if we have enough keyframes to build submaps
            for i in range(1, len(self.keyframes) - 2):
                if self.keyframes[
                    i
                ].redo_submap:  # check if the keyframe needs to be rebuilt
                    self.keyframes[i].submap = self.get_points(
                        [i - 1, i, i + 1], i
                    )  # push the submap
                    _, self.keyframes[i].ring_key = self.get_scan_context(
                        self.keyframes[i].submap
                    )  # push the ring_key
    ...
    def build_ring_key(self) -> None:
        ...
        if len(self.keyframes) >= 3:
            ...
            (
                self.keyframes[-2].context,
                self.keyframes[-2].ring_key,
            ) = self.get_scan_context(self.keyframes[len(self.keyframes) - 2].submap)

        # we are not far enough along, push in a fake polar image and ring key
        else:
            polar_image = np.ones((self.sc_bearing_bins, self.sc_range_bins)) * float(
                "inf"
            )
    ...
    def publish_newest_ring_key(self) -> None:
        ...
        if len(self.keyframes) >= 3:
            ...
            msg = RingKey()
            msg.data = list(
                np.array(self.keyframes[-2].ring_key).astype(np.uint8)
            )  # the ring key
    ...
    def check_job(self, their_keys: list, vin_list: list) -> bool:
        ...
        for their_key, search_vin in zip(their_keys, vin_list):
            if (
                self.message_pool[search_vin].keyframes[their_key].context is None
            ):  # do we have context yet?
                for i in range(
                    their_key - 1, their_key + 2
                ):  # loop over the frames needed for context
                    if (
                        i >= len(self.message_pool[search_vin].keyframes)
                        or self.message_pool[search_vin].keyframes[i].points is None
                    ):
                        return False  # we do not have the required data for this job
                self.message_pool[search_vin].update_scan_context(
                    their_key
                )  # update the scan context image
    ...
    def run_global_registration(
        ...
        for my_key in my_keys:  # loop over each of my keys
            for their_key, search_vin in zip(
                their_keys, vin_list
            ):  # loop over the outside key and the vin associated with that key

                # Check if we have run this job before, no repeats
                if (search_vin, their_key, my_key) not in self.tested_jobs:
                    self.tested_jobs[
                        (search_vin, their_key, my_key)
                    ] = True  # log that we have run this job

                    result, status, message = message_pool_copy[
                        search_vin
                    ].compare_frames(
                        my_keyframes[my_key],
                    ...
                    if status:
                        ...
                        (
                            ...
                            pose_global,
                            covariance,
                        ) = result  # parse out the results
                        cost.append(overlap)  # log the cost, in this case overlap
                        loop = ICPResultInterRobot(
                            my_key,  # push the results into a ICP results object
                            their_key,
    ...
    def publish_multi_robot_registration(self, points_for_publish: np.array) -> None:
        ...
        if points_for_publish is not None:
            points_for_publish = np.column_stack(
                (
                    points_for_publish[:, 0],
    ...
    def vis_pcm_result(self, pcm: list, vin: int) -> np.array:
        ...
        for i in pcm:
            if (
                self.multi_robot_queue[vin][i].inserted == False
            ):  # only vis a result once
                pts = (
                    self.message_pool[vin]
                    .keyframes[self.multi_robot_queue[vin][i].target_key]
                    .points
                )
    ...
    def merge_queue(self, pcm: list, vin: int) -> None:
        ...
        self.merged_robots[
            vin
        ] = True  # log that we have merged this robot into our system
                ...
                if (vin, target_key) not in self.outside_frames_added:
                    ...
                    self.keyframes_multi_robot.append(
                        Keyframe(
                            ...
                            source_pose=self.message_pool[vin]
                            .keyframes[target_key]
                            .source_pose,
                            index=target_key,
                            vin=vin,
                            index_kf=source_key,
                    ...
                    self.values.insert(
                        self.to_symbol(target_key, vin), target_pose
                    )  # insert the intial guess

    def publish_loop_closure(
        self, source_key: int, target_key: int, pose_between: gtsam.Pose2, vin: int
    ) -> None:
        ...
        msg.cloud = n2r(
            np.c_[
                self.keyframes[source_key].points,
                np.zeros_like(self.keyframes[source_key].points[:, 0]),
            ],
            "PointCloudXYZ",
        )
    ...
    def merge_trajectory(self, isam: gtsam.ISAM2, vin: int) -> gtsam.ISAM2:
        ...
        for i in range(len(self.message_pool[vin].keyframes) - 1):
            ...
            if (vin, i) not in self.outside_frames_added:
                values.insert(
                    self.to_symbol(i, vin),
                    self.message_pool[vin].keyframes[i].guess_pose,
                )  # add the initial guess

        # handle the last frame, we just need an initial guess here
        if (
            run == True and (vin, i + 1) not in self.outside_frames_added
        ):  # check if this frame is added via ICP
            self.message_pool[vin].keyframes[i + 1].guess_pose = self.home[vin].compose(
                self.message_pool[vin].keyframes[i + 1].source_pose
            )
            values.insert(
                self.to_symbol(i + 1, vin),
                self.message_pool[vin].keyframes[i + 1].guess_pose,
            )  # add the initial guess for the last frame
    ...
    def update_factor_graph(self, keyframe: Keyframe = None):
        ...
        if len(self.keyframes_multi_robot) != 0:
            ...
            for robot in self.robots:  # loop over the robots in the system
                if self.home[robot] is not None:  # check if this robot has been merged
                    isam = self.merge_trajectory(
                        isam, robot
                    )  # merge the robots trajectory into my graph
            values = (
                isam.calculateEstimate()
            )  # get the pose graph estimate using the combined graph

            # update our multi-robot keyframes
            for y in range(len(self.keyframes_multi_robot)):
                if (
                    self.home[self.keyframes_multi_robot[y].vin] is not None
                ):  # check for merge
                    # grab and update the pose
                    pose = values.atPose2(
                        self.to_symbol(
                            self.keyframes_multi_robot[y].index,
                            self.keyframes_multi_robot[y].vin,
                        )
                    )
            ...
            for robot in self.robots:
                if (
                    self.home[robot] is not None
                ):  # check if we have a reference frame for the other robot
                    partner_trajectory = []
                    for y in range(
                        0, len(self.message_pool[robot].keyframes)
                    ):  # loop over all the keyframes for this robot
                        if values.exists(self.to_symbol(y, robot)):
                            pose = values.atPose2(
                                self.to_symbol(y, robot)
                            )  # get the pose
                            self.message_pool[robot].keyframes[
                                y
                            ].guess_pose = (
                                pose  # set this pose as the new intitial guess
                            )
                            partner_trajectory.append(
                                g2n(pose)
                            )  # log it to the vis trajectory
                    self.partner_trajectory[robot] = np.array(
                        partner_trajectory
                    )  # push it to the table that stores all the traj
                ...
                # only try to get a reference frame if we have the data to do so
                    ...
                    for y in range(
                        len(self.keyframes_multi_robot)
                    ):  # loop over all the robot keyframes
                        ...
                        if self.keyframes_multi_robot[y].vin == robot:
                            ...
                            between = self.keyframes_multi_robot[y].source_pose.between(
                                gtsam.Pose2(0, 0, 0)
                            )  # estimate of robots own frame
                            home_arr[robot].append(g2n(pose.compose(between)))
                    if len(home_arr[robot]) != 0:
                        self.home[robot] = gtsam.Pose2(
                            np.mean(np.array(home_arr[robot]), axis=0)
                        )
        ...
        for x in range(len(self.keyframes)):
            ...
            errors.append(
                g2n(self.keyframes[x].pose.between(pose))
            )  # log the change in our pose estimates
        ...
        if len(errors) > 0:
            ...
            if self.mrr_resend_translation != -1:
                if (
                    euclidan_change >= self.mrr_resend_translation
                    or rotation_change >= self.mrr_resend_rotation
                ):
    ...
    def publish_partner_trajectory(self) -> None:
        ...
        for robot in self.robots:
            for x, kf in enumerate(self.partner_trajectory[robot][1:], 1):
                p1 = (
                    self.partner_trajectory[robot][x - 1][0],
                    self.partner_trajectory[robot][x - 1][1],
                    0.0,
                )
                p2 = (
                    self.partner_trajectory[robot][x][0],
                    self.partner_trajectory[robot][x][1],
                    0.0,
                )
        ...
        if len(self.keyframes_multi_robot) > 0:
            for key in range(len(self.keyframes_multi_robot)):
                ...
                if transf_points is not None:
                    ...
                    all_keys.append(
                        np.ones((len(transf_points), 3))
                        * colors[self.keyframes_multi_robot[key].vin]
                    )
    ...
    def publish_multi_robot_constraints(self) -> None:
        ...
        for frame in self.keyframes_multi_robot:
            ...
            p2 = (
                self.keyframes[frame.index_kf].pose3.x(),
                self.keyframes[frame.index_kf].pose3.y(),
                self.keyframes[frame.index_kf].dr_pose3.z(),
            )
    ...
    def filter_by_distance(
        ...
        distances_euclidian = np.linalg.norm(
            distances_euclidian[:, :2], axis=1
        )  # Parse out ecuclidan distance
        # filter based on max distance and rotation
        indexes = indexes[
            (distances_euclidian < self.mrr_max_translation_search)
            & (rotations < self.mrr_max_rotation_search)
        ]
        distances = distances[
            (distances_euclidian < self.mrr_max_translation_search)
            & (rotations < self.mrr_max_rotation_search)
    ...
    def filter_by_distance_2(
        ...
        for idx, vin in zip(indexes, vins):  # loop over the Kd-tree nearest neighbors
            ...
            if (
                self.home[vin] is not None
                and self.message_pool[vin].keyframes[idx].source_pose is not None
            ):
                pose_2 = self.home[vin].compose(
                    self.message_pool[vin].keyframes[idx].source_pose
                )  # their pose in my frame
                distances_euclidian.append(
                    g2n(my_pose.between(pose_2))
                )  # pose between these

            # we have not merged this robot yet, but we do have a source pose for it, do not filter
            elif (
                self.home[vin] is None
                and self.message_pool[vin].keyframes[idx].source_pose is not None
            ):
                distances_euclidian.append(
                    g2n(gtsam.Pose2(0, 0, 0))
                )  # push through zeros so this does not get removed

            # not enough information
            else:
                distances_euclidian.append(
                    g2n(gtsam.Pose2(500000, 500000, 500000))
                )  # push a big number so it does get removed
